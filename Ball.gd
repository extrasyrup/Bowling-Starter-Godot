...
func ball_left():
	print ("Ball Left")
	global_translate(Vector3(0, 0, -.2)) #Shift ball position left

func ball_right():
	print ("Ball Right")
	global_translate(Vector3(0, 0, .2)) #Shift ball position right
...
func ball_push():
	...
	apply_impulse(Vector3(0, .1, 0), get_global_transform().basis.z * 80) #Impulse to push the ball in facing direction
